...
func (s *EpmsServer) RegisterService(serviceName string, svc interface{}, metaData string) error {
	//todo
	return protocols.Inprocessor.Register(serviceName, svc, metaData)
}

func (s *EpmsServer) UnRegisterService(serviceName string) error {
	//todo
	return protocols.Inprocessor.Unregister(serviceName)
}

func (s *EpmsServer) SystemProcessor(ctx context.Context, servicePath, serviceMethod string, args interface{}, reply interface{}) *protocols.Call {
	return protocols.Inprocessor.Go(ctx, servicePath, serviceMethod, args, reply, nil)
}

func (s *EpmsServer) BusinessProcessor(ctx context.Context, servicePath, serviceMethod string, args interface{}, reply interface{}) *protocols.Call {
	return protocols.Inprocessor.Go(ctx, servicePath, serviceMethod, args, reply, nil)
}
...
func (s *EpmsServer) DistributingMsg(addr string, data []byte) {
	...
	epmsBody := &nc.NcEPMSMsgHeader{}
	s.thrift.GetStructValue(epmsBody, data)
	//2.get msg type
...
func (s *EpmsServer) BusinessHandler(addr string, msg hub.Message) {
	...
	epmsBody := new(nc.NcEPMSMsgHeader)
	err := s.thrift.GetStructValue(epmsBody, data)
	if err != nil {
	...
	if len(urlPath) > 0 && strings.Contains(urlPath, "/") {
		...
		if len(services) == 2 {
			...
			res := s.BusinessProcessor(context.Background(), servicePath, methodName, args, reply)
			log.Info("%v", res)
			err, buffer := s.thrift.GetStructBuffer(reply)
			if err != nil {
				log.Error("【BusinessHandler】%v", err)
			}
			...
			for e := s.conn.Front(); e != nil; e = next {
				...
				c := e.Value.(*connection.Connection)
				if c.PeerAddr() == addr {
					err, sendData := s.thrift.GetStructBuffer(epmsBody)
					if err != nil {
...
func (s *EpmsServer) SystemHandler(addr string, msg hub.Message) {
	...
	//4.send client
	data := msg.Fields["data"].([]byte)
	var epmsBody *nc.NcEPMSMsgHeader
	err := s.thrift.GetStructValue(epmsBody, data)
	if err != nil {
		log.Error("【BusinessHandler】%v", err)
	}
	...
	if len(urlPath) > 0 && strings.Contains(urlPath, "/") {
		...
		if len(services) == 2 {
			...
			methodName := services[1]
			args, reply := s.thrift.GetArgsAndReply(epmsBody)
			res := s.SystemProcessor(context.Background(), servicePath, methodName, args, reply)
			log.Info("%v", res)
			err, buffer := s.thrift.GetStructBuffer(reply)
			if err != nil {
